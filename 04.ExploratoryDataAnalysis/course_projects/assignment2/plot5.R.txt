...
dat_Maryland <- data_merge[which(data_merge$fips == "24510"),]

trend <- ddply(dat_Maryland, .(year), summarise, total_emission_year=sum(Emissions))

png("plot5.png")
plot(trend$year, trend$total_emission_year, main="Total PM2.5 emissions from motor vehicle sources
     in the United States (1999-2008)", pch=19, xlab="year", ylab="Amount of PM2.5 emitted (tons)")
dev.off()
