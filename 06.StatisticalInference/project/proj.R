...
## You will create a report (PDF format) to answer the questions. Each pdf 
## report should be no more than 3 pages with 3 pages of supporting appendix 
...
clt_sd <- 1/lambda/sqrt(n)
clt_v <- clt_sd^2
clt_v
# Central Limit Theorem distribution

hist(dnorm(seq(min(df[,1], max(df[,1], length.out=1000))), clt_m, clt_sd))
...
## Confused?  Try re-watching video lecture 07 for a starter on how to complete 
## this project.

## Sample Project Report Structure
## Of course, there are multiple ways one could structure a report to address 
## the requirements above.  However, the more clearly you pose and answer each 
## question, the easier it will be for reviewers to clearly identify and 
## evaluate your work. 
...
##             the accompanying R code. Your explanations should make clear what 
...
##             explains the figures and what is shown on them, and provides 
##             appropriate numbers.
## - Sample Variance versus Theoretical Variance: Include figures (output from R) 
##            with titles. Highlight the variances you are comparing. Include 
...
## Now in the second portion of the class, we're going to analyze the 
